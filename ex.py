...
for p in musical_list.find_elements_by_tag_name('li'):
    ...
    #tag 판매중 or 종료
    try:
        tag = p.find_element_by_css_selector('div.tags > span.tag.onSale').text
        tags.append(tag)
    except:
        tag = "판매 종료"
        tags.append(tag)
    ...
    #포스터 사진
    # try:
    #     poster = p.find_element_by_css_selector('div.col.photo > a > span').get_attribute('src')
    #     musical_poster.append(poster)
    # except:
    #     musical_poster.append(None)

    #ticketContent > div > div.searchPanel > ul > li:nth-child(18) 
